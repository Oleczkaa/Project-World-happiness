# -*- coding: utf-8 -*-
"""Project_World_Happiness_shared_before_tuning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/176wQ3iC0poJTkzDH4gLyzD6dFemmgo75
"""
...
#To be able to work on the same file:
file_id_csv = "1OVIDtXz4eLLNw_LAgCGdxhdQFdboMy_U"
download_url_csv = f"https://drive.google.com/uc?id={file_id_csv}"
response_csv = requests.get(download_url_csv)
data_csv = BytesIO(response_csv.content)

file_id_excel = "1FDkS99PAsrQWQrNZ_FD6cQcHiCSbrhlS"
download_url_excel = f"https://drive.google.com/uc?id={file_id_excel}"
response_excel = requests.get(download_url_excel)
data_excel = BytesIO(response_excel.content)


# Load the CSV file into a DataFrame, df_2 is the dataFrame with Regions (source: WHO)
df = pd.read_excel(data_excel)
df_2 = pd.read_csv(data_csv, sep=";")

#checking data type of each column
df.info()
print("\n")

#shows the procentual value of every column
nan_per = (df.isna().sum() / len(df)) * 100

#NaN percentage per column
print("Procental missing value of the column:\n", nan_per.round(2))
print("\n")
#checking what years are included in the dataframe
df["year"].unique()
...
#to check how many categories this columns has:
category_counts = df[categorical_columns].nunique()
print("how many categories", category_counts)

#Check for duplicates
print("There are", df.duplicated().sum(), "duplicates in df dataset")

"""# Neuer Abschnitt"""

#Merge of df and df_2

df_2.rename(columns={"Area": "Country name"}, inplace=True)
df_all = pd.merge(df, df_2, on=['Country name'])

df_all.head()

# Speichern als CSV
df_all.to_csv("mein_datensatz.csv", index=False)
import os
...
countries_per_year = df.groupby('year')['Country name'].nunique().reset_index()
...
missing_data = df.pivot_table(index='Country name', columns='year', aggfunc='size', fill_value=0)

# Calculate the number of missing values per country per year
missing_counts = df.groupby(['Country name', 'year']).size().unstack(fill_value=0)
